...
def make_sc(files: dict = None):
    """
    files: {path: bytes_content} — จำลอง HDFS file system
    """
    files = files or {}
    sc = MagicMock()

    written = {}  # path -> bytes ที่เขียนผ่าน fs.create
    ...
    def exists(path_mock):
        return path_mock.toString() in files or path_mock.toString() in written

    def fs_open(path_mock):
        p = path_mock.toString()
        content = files.get(p) or written.get(p) or b""
        stream = MagicMock()
        pos = [0]

        # simulate Java byte array read
        def read(buf):
        ...
        # simulate BufferedReader for text files
